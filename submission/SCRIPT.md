...
> ⭐ **2026-08-30 — 이 대본으로 영상을 실제로 만들었다.**
> 산출물 `warranty-demo.mp4` (3:27 · 1920x1080 · 영어). 촬영 기록과 오늘 받은 실물 출력은
> `docs/evidence/demo-video-2026-08-30.log`에 있고, 내레이션 대본은 `submission/vo/*.txt`다.
>
> ⚠️ **내레이션은 기계 음성(macOS `say`)이다.** 사람 목소리로 갈아끼우려면 `vo/*.txt`를
>    읽어 녹음해서 같은 이름으로 넣고 `assemble.py`를 다시 돌리면 된다 —
>    **길이는 오디오에서 다시 읽으므로 타이밍을 손으로 맞출 필요가 없다.**
>
> ⚠️ **실제 데스크톱 촬영은 버렸다.** Chrome 창이 여럿이라 프레임에 개인 창이 들어왔다.
>    화면은 전부 headless 렌더링이고, 원장 화면은 **배포된 공개 URL을 그대로** 찍은 것이다.
