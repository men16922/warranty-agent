...
⚠️ 원장 행을 만드는 계량이 `ModelCallMeter` **한 겹에만** 있는 것이 요점이다. 판정 모델은
   `MeteredModel`이 감싸고, ADK는 응답 이벤트의 usage를 넘긴다. 호출부마다 원장 모양을
   다시 만들지 않으므로 `Remediator`와 ADK Runner는 원장 불변식을 모른다.
"""
...
class ModelCallMeter:
    """SDK가 돌려준 사용량 하나를 모델 호출 원장 행 하나로 옮긴다."""
    ...
    def record(self, action_id: str, usage: TokenUsage | None) -> None:
...
class MeteredModel:
    ...
    def _call(self, action_id: str, invoke: Callable[[], ModelReply]) -> ModelReply:
        ...
        try:
            ...
            self._meter().record(action_id, usage)
        return reply

    def _meter(self) -> ModelCallMeter:
        return ModelCallMeter(self.ledger, self.clock, self.ids, self.prices, self.agent_id)
