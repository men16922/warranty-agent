...
> T2-2(배포) → T3(계약 방출). 그전까지 오프라인으로 가능한 건 **REQ-803 · REQ-804** 둘뿐이다
> (REQ-404 · REQ-405 · REQ-508 · REQ-603은 2026-08-21에 끝났다).
...
```
게이트    make check → 108 passed (2026-08-21 로컬 macOS · py3.13)
...
⚠️ **남은 13개 TODO 대부분이 실물 GCP를 요구한다.** 오프라인으로 더 갈 수 있는 것은
[`tasks.md`](../specs/warranty/tasks.md)에서 **`[auto]`가 붙은 것**이 권위다 —
지금 남은 둘은 T6-2(REQ-803) · T6-3(REQ-804)이고,
**둘을 다 해도 중단 기준의 판정 대상(T2-2)은 움직이지 않는다.**
