...
> ✅ **영상이 나왔다**(08-30) — `submission/warranty-demo.mp4` · 3:49 · 영어.
> 남은 것은 **영상 공개 링크 확보 → DEVPOST에 박기 → 제출**, 그리고 teardown(09-02)이다.
> ⚠️ teardown 범위가 **여섯**이 됐다 — 촬영 중 `day1-demo-final`이 생겼다(T8-6).
> ✅ **논지의 세 다리가 전부 공개 URL에서 실물이다**(리비전 `00007-mrq` · `d51566c`):
> 검증 · 원자적 롤백 · **조치당 비용**. 셋째는 08-29 오전까지 통째로 비어 있었다.
> ⭐ **화면**: `GET /`가 원장을 사람이 읽는 표로 낸다 — 읽기 전용, 버튼 없음, HTTP 200 HTML.
> ⭐ **라벨이 실물이다**: `fl_entry=01m16hev85z5b7b4ykzc73tm95`이 GCP 리소스에 박혔고
> 원장 항목 id와 같다 — **되읽어 확인했다**(`evidence/live-cost-axis-2026-08-29.log`).
...
> ✅ **P0 결과 · 갈래 ⓒ 확정**: `warranty-hack`에 **L4 GPU 쿼터가 없다**
...
> ⭐ **조치가 둘이고 둘 다 실물이다**(T2-5): `concurrency:16`이 새 리비전
> `demo-target-00003-67d`를 만들고 `674.17 → 988.60` → `not_recovered` → 롤백까지 완주했다.
> **롤백 코드는 한 줄도 안 늘었다** — 이미 원자적이라고 되읽어 증명한 그 경로다.
> ✅ 영상(T8-3) 완료 — 촬영 기록 `docs/evidence/demo-video-2026-08-30.log`.
> ⚠️ **촬영이 화면의 결함을 두 개 잡았다**: 화면이 한국어였고, 표가 잘려 `Cost`·`Reason`이
>    가로 스크롤 뒤에 숨어 있었다. 둘 다 고쳐서 재배포했다(`00009-2lk`).
> ✅ **Day-1·Day-2·리포트·화면이 전부 공개 URL에서 실물이다**(`00007-mrq` · `d51566c`) — 만들고
