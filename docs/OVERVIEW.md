...
> 열리면 순서는 T0-3 → **T2-1**(ADK 실물 호출 — 임포트는 확인됐고 **호출은 아직**) →
> T2-2(배포) → T3(계약 방출). ⛔ **오프라인으로 갈 수 있는 곳은 다 갔다** — 2026-08-21에
> REQ-404 · REQ-405 · REQ-508 · REQ-603 · REQ-803 · REQ-804가 끝나면서
> `[auto]` 백로그가 **말랐다**. 남은 것은 전부 실물 GCP이거나 사람 판단이다.
> ⚠️ `.env`의 `WR_PROJECT_ID`는 **T2-1 스모크용 부트스트랩 값**이지 전용 프로젝트가 아니다.
...
```
게이트    make check → 120 passed (2026-08-21 로컬 macOS · py3.13)
요구사항  44종 — VERIFIED 18 · IMPLEMENTED 14 · TODO 12
가드      G1~G4 · G6~G9 변이 확인 · G5만 미착수
변이      M-01~M-32 전부 red 확인 · 복구 후 초록 · 잔여 0
...
⚠️ **남은 TODO는 대부분 실물 GCP를 요구한다.** 오프라인으로 더 갈 수 있는 것은
[`tasks.md`](../specs/warranty/tasks.md)에서 **`[auto]`가 붙은 것**이 권위다 —
⛔ **지금은 하나도 안 남았다.** 그리고 그것을 다 했어도
**중단 기준의 판정 대상(T2-2)은 움직이지 않았다.**
⚠️ 여기에 숫자를 다시 세어 적지 않는다 — 세어 적은 숫자는 **기계가 안 읽어서 조용히 썩는다**
(T0-6의 교훈).
