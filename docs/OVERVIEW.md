...
> **남은 필수 산출물이 없다.** 코드·배포·영상·아티클·갤러리 전부 나왔고 push 끝났다(`d0d012f`).
> 남은 것은 사람 손이다: Medium 발행 · Devpost 폼 · teardown(**09-02 · 서비스 여섯**).
> ⚠️ `submission/SCRIPT.md`가 아직 **옛 논지의 대본**이다 — 산출물과 다른 이야기를 한다.
>
> ⭐ **논지는 함대 회계다**(08-30 재구성 · D16 · `HACKATHON.md` §0) —
> *"에이전트 함대가 밤새 뭘 했는지 아침에 어떻게 아는가."* 코드는 0줄 바뀌지 않았다.
> ⛔ **이 시스템이 자기 논지에 네 번 걸렸고, 넷 다 로그로는 초록이었다**(08-30).
> 원인·증거는 [`evidence/verify-window-2026-08-30.log`](evidence/verify-window-2026-08-30.log).
> ⚠️ **로그로 못 찾았다 — 어긋나면 안 되는 두 숫자를 노려보다 찾았다.**
>
> ⭐ **같은 조치, 두 판정**: `990.04 → 674.17ms` **recovered** / `674.17 → 990.04ms`
> **not_recovered** → 자동 롤백 → 트래픽 되읽어 확인. **원장이 어느 쪽인지 말한다.**
> ⭐ 원장 실물 `executed 14 · improved 1 · rolled_back 12 · manual_required 3`.
> ⚠️ 롤백 대부분은 우리가 시험하며 만든 것이다 — **비율을 예쁘게 하려고 원장을 비우지 않는다.**
> ⚠️ **조치 행의 금액은 0이고 그게 맞다.** 말할 수 있는 것은 금액의 크기가 아니라
> **그 수를 청구서에서 되찾을 수 있는가**(귀속)다.
>
> ⭐ 실물: `warranty-api`(공개 · `00012-487` · **`/`에 영어 원장 화면**) ·
> 영상 https://youtu.be/KAcHpX3nSSM · 상세는 §10과 [`tasks.md`](../specs/warranty/tasks.md).
...
루프      ⭐ 완주. **양쪽 판정이 다 실물이다** — `recovered`와 `not_recovered`+롤백(08-30)
검증      ⛔ 08-30에 결함 넷을 회수했다 → `evidence/verify-window-2026-08-30.log` · D17
조치      ⭐ **둘 다 실물이다** — 트래픽 전환 + 동시성 변경. 롤백 코드는 한 줄도 안 늘었다
          ⛔ 롤백은 **트래픽이지 템플릿이 아니다** — 되돌린 뒤에도 템플릿 동시성은 16이다
화면      ⭐ `GET /` — 원장의 읽기 뷰가 프로덕션에서 200 HTML. **영어** · 읽기 전용·버튼 없음
비용      ⭐ 모델 호출 `token_meter · $0.00174675` · 프로비저닝 `resource_label`(라벨 되읽음)
서버      ⭐ 배포 `00012-487`이 트래픽 100% · 요청 타임아웃 540s(검증 대기 2×135s를 견딘다)
...
**다음**: **제출(09-01 09:00 KST) → teardown(09-02).**
...
크레딧(Free Trial) **만료 2026-09-06**. 제출 09-01 · **teardown 09-02**(서비스 여섯) 뒤
여유가 **4일뿐**이다. ⇒ T8-6은 *"나중에"*가 아니라 **날짜가 박힌 일**이다.
✅ 중단 기준(*"08-24까지 Cloud Run에서 도는 것이 없으면 접는다"*)은 08-23 배포로 통과했다.
