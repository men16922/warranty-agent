...
> ▶ **NEXT SESSION: 새 리비전을 배포하고(UI + 비용 라벨) 실물 확인 → 부하 켠 채 영상(T8-3).**
> ⭐ **화면이 생겼다**: `GET /`가 원장을 사람이 읽는 표로 낸다 — 읽기 전용, 버튼 없음.
> ⭐ **비용 축이 실물이 됐다**(계획 `plans/2026-08-29-cost-axis.md`): 공시 단가표(T5-5)로
> 모델 호출이 금액을 갖고, 프로비저닝이 **원장 행 + `fl_entry` 라벨**을 남긴다(T5-6) —
> `Method.RESOURCE_LABEL`은 2026-08-29까지 **테스트에만 있었다.**
...
화면      ⭐ `GET /` — 원장의 읽기 뷰. `실행됨` 옆에 `나아짐`, 금액 옆에 **귀속**이 있다
비용      ⭐ 단가표(T5-5) + 프로비저닝의 원장 행·라벨(T5-6). ⛔ 아직 **배포 전**이다
