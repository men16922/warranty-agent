...
def _decide(rev: Reversibility, verifiable: bool, **over: object) -> object:
...
def test_req_402_every_cell_of_the_matrix_is_reached_by_value(
    ...
    """
    assert _decide(reversibility, verifiable).verdict is expected  # type: ignore[attr-defined]
...
def test_req_402_no_headroom_denies_regardless_of_the_other_axes() -> None:
    ...
    for rev in Reversibility:
        for verifiable in (True, False):
            ...
            assert d.verdict is Gate.DENY  # type: ignore[attr-defined]
...
def test_req_402_unverifiable_can_never_be_auto() -> None:
    ...
    for rev in Reversibility:
        for projected in (Decimal("0.00"), CHEAP, HEADROOM):
            ...
            assert d.verdict is not Gate.AUTO, (  # type: ignore[attr-defined]
...
def test_req_406_destructive_actions_are_never_auto() -> None:
    ...
    assert d.verdict is Gate.APPROVE  # type: ignore[attr-defined]
...
def test_req_401_decision_records_which_cell_it_came_from() -> None:
    ...
    assert d.rule == "reversible but not verifiable"  # type: ignore[attr-defined]
    assert d.verifiable is False  # type: ignore[attr-defined]
