...
- [ ] [auto] **T11-2** REQ-805 가드 *(T11-1 선행)* · `Implements: REQ-805`
      "상시 과금 컴퓨트를 만들지 않는다" — 지금까지는 **훑을 대상이 0개라 공허 통과**였다.
      T11-1이 매니페스트를 만들면 비로소 집행 가능하다.
      `Done:` `min-instances>0`을 넣으면 red + 변이 확인.
...
**게이트**: `make check` → **176 passed** (2026-08-22 로컬 macOS·py3.13)
