...
- [ ] [auto] **T11-3** 의존성 선언 정합성 — ⛔ **`google-adk`가 어디에도 선언돼 있지 않다**
      (REQ-601은 대회 필수인데). `pyproject.description`도 구 정체성이다.
      `Done:` 선언 + description 현행화 + **선언과 실제 임포트의 어긋남을 묻는 가드**.
      ⚠️ 설치는 네트워크다 — 선언만 하고 게이트는 그것 없이 통과해야 한다(cloud extra의 전제).
...
**게이트**: `make check` → **180 passed** (2026-08-22 로컬 macOS·py3.13)
