...
- [ ] [auto] **T11-1** 배포 산출물 — `Dockerfile` + 배포 스크립트(실행 안 함).
      서비스명·리전·`min-instances=0`이 `src/warranty/config.py`와 **한 곳에서** 온다.
      `Done:` 산출물 존재 + 설정과의 불일치를 게이트가 red로 잡는다. **배포는 안 한다.**
...
**게이트**: `make check` → **169 passed** (2026-08-22 로컬 macOS·py3.13)
