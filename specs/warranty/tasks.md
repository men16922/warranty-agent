...
- [ ] [auto] **T10-2** 상태 칸이 증거와 어긋난 4건 — `REQ-206·305·801·802`는 변이 기록이
      **있는데** `IMPLEMENTED`다. 기록이 그 요구사항의 **문장 전부**를 덮는지 확인해
      덮으면 `VERIFIED`로, 안 덮으면 부족한 변이를 채운다.
      `Done:` 넷 다 상태와 증거가 일치 + 근거 한 줄씩.
...
**게이트**: `make check` → **167 passed** (2026-08-22 로컬 macOS·py3.13)
