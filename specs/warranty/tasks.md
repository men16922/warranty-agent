...
- [ ] **T5-3** 제출·실물 요건 셋에 `Verifies:`를 단 테스트를 붙여 상태 칸을 현실과 맞춘다.
  ⛔ **셋 다 사실은 참인데 게이트가 `TODO`를 강제한다** — 겨냥한 테스트가 0이기 때문이다.
  게이트가 틀린 게 아니라 겨냥한 자리가 없다. 오프라인에서 확인 가능한 절반을 겨눈다:
  - `REQ-602` 배포 계획이 `min-instances=0`을 렌더하는가 · 진입점이 포트를 여는가 (실물 절반은
    `docs/evidence/live-agent-chat-2026-08-29.log`가 이미 갖고 있다)
  - `REQ-902` 선언에 `git+`·`file://`·사설 인덱스가 없는가 (T8-4가 손으로 확인한 것을 집행으로)
  - `REQ-901` 제출물 일체 — README가 가리키는 링크가 실재하는가
  ⚠️ `VERIFIED`까지 가려면 변이 기록이 함께 필요하다. 못 겨누는 절반은 **겨눴다고 하지 않는다.**
  `Implements: REQ-602, REQ-901, REQ-902` · `Design: 10§5, 10§8`
