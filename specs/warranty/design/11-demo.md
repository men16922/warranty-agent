...
Cloud Monitoring의 p95는 요청이 있어야 구간을 구별한다. 2026-08-23 실물 실험에서 구간당
3회는 점 2개·같은 값으로 뭉쳤고, **40회**에서 674/989/674ms로 갈라졌다. 따라서 라이브
각본은 다음 세 구간의 부하를 값으로 갖는다.

```json
{"baseline": 40, "after_action": 40, "after_rollback": 40}
```

⚠️ 오프라인 `make demo`는 이 계획을 **출력만** 하고 요청은 보내지 않는다(REQ-801).
실물 배선이 같은 값을 소비해 요청을 보내는 것은 T2-4의 남은 절반이다.
