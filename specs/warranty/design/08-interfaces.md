...
### 2.1 Firestore 문서 — **저장이 성공했다는 것과 값이 살아 있다는 것은 다르다**

`ContractStore`·`LedgerStore`의 실물은 Firestore Native다(design 10§2). 컬렉션은 둘이고
문서 id는 도메인 id를 그대로 쓴다 — `contracts/{contract_id}` · `ledger/{entry_id}`.

| 도메인 | 문서에 실리는 모양 | 왜 |
|---|---|---|
| `Decimal` | **문자열** | Firestore의 수치는 double이다. 돈을 double로 왕복시키면 `0.0200`이 다른 값으로 돌아오고, **자릿수**(단가를 몇 자리로 계산했는가)도 사라진다 (REQ-503·505) |
| `datetime` | ISO-8601 문자열 | 타임존과 마이크로초를 그대로 되읽는다 |
| `StrEnum` | 선언된 값 | 모르는 값은 **복원에서 거절한다** — 조용히 문자열로 두면 상태가 열거형 밖으로 샌다 |
| `init=False` 필드 | **안 싣는다** | 파생 선언을 저장하면 도메인이 그것을 고치는 날 옛 문서가 새 코드와 다른 것을 말한다 |

⛔ **필드는 손으로 옮기지 않는다.** 손으로 적으면 도메인에 필드가 하나 느는 날 그 필드만
조용히 저장 안 되고, 행은 여전히 읽히며 빠진 값은 기본값으로 채워진다. 그 실패는 예외가
아니라 침묵이다. ⇒ 데이터클래스의 필드를 **타입으로 훑어** 싣고, 되읽을 때 **빠진 키도
남는 키도 거절한다.** 마이그레이션은 소리를 내야 한다.

⛔ **불변식은 어댑터에 없다.** 무엇이 허용되는지는 `warranty.domain.entry`의 전이 함수
(`apply_approval`·`apply_completion`·`apply_reconcile`·`apply_give_up_reconcile`)가 알고,
저장소는 읽고 쓰기만 한다. 저장소가 둘인데 규칙이 두 벌이면 한쪽만 고쳐지는 날이 오고,
그날 원장은 **저장소에 따라 다른 것을 허용한다**(§2).

⚠️ **1회 = 1행은 Firestore가 집행한다** — 읽어 보고 없으면 쓰는 대신 `create()`를 쓴다.
읽고-쓰기 사이의 창에서 같은 id가 두 번 들어오면 그 규칙은 관례가 된다 (REQ-501).
전이는 **트랜잭션 안에서** 읽고-바꾸고-쓴다 — 밖에서 하면 나중 쓰기가 앞선 쓰기를 말없이 덮는다.

⚠️ **`retired`된 계약은 질의에서 거른다**(`state == active`). 파이썬에서 거르면 죽은 계약이
한 번은 메모리에 올라오고, 그것을 거르는 줄이 사라지는 날 자동 조치가 **없는 리소스를
고치려 든다**(REQ-105). 한 리소스에 살아 있는 계약이 둘이면 **거절한다** — 아무거나 고르면
*"어느 기준으로 회복을 판정했는가"*의 답이 실행할 때마다 달라진다(REQ-102).
...
✅ **그 경계 뒤에 이제 내용이 있다.** 유효한 토큰이면 `/agent:chat`은 `200`이고 ADK 단발
세션이 돈다(2026-08-28 프로덕션 확인 · `docs/evidence/live-agent-chat-2026-08-28.log`).
콜백이 주입되지 않으면 여전히 `501`, 호출이 실패하면 `503`이다 — **fake 성공은 만들지 않는다.**

⛔ **`/actions/{action_id}:remediate`는 아직 `501`이고, 그것은 미구현이 아니라 결정 대기다.**
경로를 열려면 **공개 URL에 과금·변경 엔드포인트를 노출**하게 된다 — 이 절이 지금 인증을
`/agent:chat` 하나에만 두기 때문이다. 렌더러(`wire.remediate_response`)와 실물 경로는 이미
있으므로 남은 것은 코드가 아니라 **경계를 어디까지 넓힐지의 결정**이다(tasks.md T5-1).
