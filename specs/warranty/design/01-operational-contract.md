...
- ⛔ **§3의 표와 코드가 어긋나는 자리가 하나 있다 — `reversibility`.** 표는
  *"리소스 타입에서 유도 (Cloud Run 서비스 = 가역)"*라고만 적지만, 코드
  (`usecases/provision.py`의 `derive_contract`)는 **타입 ∧ 돌아갈 자리**를 함께 본다.
  타입만 보면 **되돌아갈 리비전이 없는 첫 배포**가 `REVERSIBLE`이 되고, 그 계약은
  롤백 계획이 없어 **REQ-102가 거부한다** — 계약이 아예 성립하지 않는다.
  그래서 응답에 직전 리비전이 없으면 `IRREVERSIBLE`이고, 그 리소스는 자동 대상이 아니다.
  ⚠️ **표를 고치는 것은 설계 변경(사람 판단)이라 하지 않았다.** 어긋남을 지우지 않고
  여기 적어 둔다 — 코드 쪽 근거는 `docs/evidence/mutations.md`의 M-56이다.
